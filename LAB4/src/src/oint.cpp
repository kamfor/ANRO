...
bool interpolatePose(lab4::oint_control_srv::Request& request, lab4::oint_control_srv::Response& response){
    ...
    if(request.type == "linear"){
      ...
      for(int i=0; i<steps; i++){ 
        ...
        arx = calculatePoly(rx, rox, t_act, t);
        ary = calculatePoly(ry, roy, t_act, t);
        arz = calculatePoly(rz, roz, t_act, t);
